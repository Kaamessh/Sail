...
function setupEventListeners() {
  ...
  if (originSelect && destSelect) {
    originSelect.addEventListener('change', () => {
      // If haldia selected, advise user
      checkDraftAlerts();
    });
...
function updateExecutiveKPIs(market) {
  ...
  const bdiClose = market.BDI_Close || 1850;
  const bdi7dMa = market.BDI_7D_MA || Math.round(bdiClose * 0.995);
  const bdiVol = market.BDI_30D_Vol || 28.5;
  const vlsfo = market.Bunker_VLSFO || 585.0;
  const mgo = market.Bunker_MGO || 760.0;
  const ifo = market.Bunker_IFO380 || 430.0;
  const hi5 = market.Hi5_Spread || (vlsfo - ifo);
...
async function fetchForecastCones(marketSnapshot) {
  try {
    const payload = {
      BDI_Close: marketSnapshot.BDI_Close || 1850,
      BDI_Open: marketSnapshot.BDI_Open,
      BDI_High: marketSnapshot.BDI_High,
      BDI_Low: marketSnapshot.BDI_Low,
      Bunker_VLSFO: marketSnapshot.Bunker_VLSFO || 585,
      Bunker_MGO: marketSnapshot.Bunker_MGO || 760,
      Bunker_IFO380: marketSnapshot.Bunker_IFO380 || 430,
    };
    ...
    if (f30) {
      ...
      // Update Strip
      document.getElementById('strip-p10').innerText = `${Math.round(f30.p10).toLocaleString()} pts`;
      document.getElementById('strip-p50').innerText = `${Math.round(f30.p50).toLocaleString()} pts`;
      document.getElementById('strip-p90').innerText = `${Math.round(f30.p90).toLocaleString()} pts`;
    }
...
function renderForecastChart() {
  ...
  if (!canvas || !state.history.length || !state.forecasts) return;
  ...
  // Build combined date labels
  const histLabels = state.history.map(h => h.date.slice(5)); // 'MM-DD'
  const lastDateStr = state.history[state.history.length - 1].date;
  const lastDate = new Date(lastDateStr);
  const baseBdi = state.currentMarket?.BDI_Close || 1850;

  const date7 = new Date(lastDate);
  date7.setDate(date7.getDate() + 7);
  const label7 = date7.toISOString().slice(5, 10) + ' (+7D)';

  const date14 = new Date(lastDate);
  date14.setDate(date14.getDate() + 14);
  const label14 = date14.toISOString().slice(5, 10) + ' (+14D)';

  const date30 = new Date(lastDate);
  date30.setDate(date30.getDate() + 30);
  const label30 = date30.toISOString().slice(5, 10) + ' (+30D)';

  const forecastLabels = [label7, label14, label30];
  const allLabels = [...histLabels, ...forecastLabels];

  // Historical Series
  const histData = state.history.map(h => h.BDI_Close);
  const paddedHist = [...histData, null, null, null];

  // Quantile Projections starting from the last historical point
  ...
  p10Data[lastIndex] = baseBdi;
  p50Data[lastIndex] = baseBdi;
  p90Data[lastIndex] = baseBdi;

  p10Data[lastIndex + 1] = state.forecasts['7D'].p10;
  p50Data[lastIndex + 1] = state.forecasts['7D'].p50;
  p90Data[lastIndex + 1] = state.forecasts['7D'].p90;

  p10Data[lastIndex + 2] = state.forecasts['14D'].p10;
  p50Data[lastIndex + 2] = state.forecasts['14D'].p50;
  p90Data[lastIndex + 2] = state.forecasts['14D'].p90;

  p10Data[lastIndex + 3] = state.forecasts['30D'].p10;
  p50Data[lastIndex + 3] = state.forecasts['30D'].p50;
  p90Data[lastIndex + 3] = state.forecasts['30D'].p90;

  // Filter based on selected horizon
  if (state.selectedHorizon === '7') {
    p10Data[lastIndex + 2] = null;
    p50Data[lastIndex + 2] = null;
    p90Data[lastIndex + 2] = null;
    p10Data[lastIndex + 3] = null;
    p50Data[lastIndex + 3] = null;
    p90Data[lastIndex + 3] = null;
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          fill: '+1', // Fill down to P10 for confidence corridor
...
function renderOptimizationResults(data) {
  ...
  recCost.innerText = `$${data.optimal_landed_cost_pmt.toFixed(2)}`;
  recSummary.innerText = data.recommendation_summary;

  // Update Benchmark KPI Card
  document.getElementById('kpi-benchmark-cost').innerText = `$${data.optimal_landed_cost_pmt.toFixed(2)}`;
  ...
  data.vessel_options.forEach(v => {
    ...
    tr.innerHTML = `
      ...
      <td>${draftBadge}</td>
      <td>${v.num_voyages}</td>
      <td>${v.duration_days.toFixed(1)} d</td>
      <td>${v.vlsfo_consumption_mt.toFixed(1)} MT</td>
      ...
      <td><strong style="color: ${isWinner ? 'var(--accent-emerald)' : 'var(--text-primary)'}; font-family: var(--font-mono); font-size: 0.95rem;">$${v.landed_cost_per_tonne.toFixed(2)}</strong></td>
      <td>${v.co2_emissions_mt.toFixed(1)} MT</td>
    `;
...
async function saveCurrentScenario() {
  ...
  const title = `${opt.cargo_tonnes.toLocaleString()} MT ${opt.origin_port} to ${opt.destination_port}`;
